...
1. Import dependencies
    ...
    ```python
    from devgpt.agentchat import GroupChat, AssistantAgent, UserProxyAgent, GroupChatManager
    from devgpt.oai.openai_utils import config_list_from_dotenv
    ```
...
## Notebook examples
More examples can be found in the [notebook](https://khulnasoft.github.io/devgpt/docs/notebooks/agentchat_groupchat_finite_state_machine/). The notebook includes more examples of possible transition paths such as (1) hub and spoke, (2) sequential team operations, and (3) think aloud and debate. It also uses the function `visualize_speaker_transitions_dict` from `devgpt.graph_utils` to visualize the various graphs.
