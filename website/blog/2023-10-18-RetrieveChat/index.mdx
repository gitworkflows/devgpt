---
title: Retrieval-Augmented Generation (RAG) Applications with DevGpt
authors: thinkall
tags: [LLM, RAG]
---

*Last update: August 14, 2024; DevGpt version: v0.2.35*

![RAG Architecture](img/retrievechat-arch.png)

**TL;DR:**
* We introduce **RetrieveUserProxyAgent**, RAG agents of DevGpt that
...
DevGpt that allows retrieval-augmented generation. The system consists of two agents: a
Retrieval-augmented User Proxy agent, called `RetrieveUserProxyAgent`, and an Assistant
agent, called `RetrieveAssistantAgent`; `RetrieveUserProxyAgent` is extended from built-in agents from DevGpt,
...
Please install pydevgpt with the [retrievechat] option before using RAG agents.
```bash
...
ragproxyagent = RetrieveUserProxyAgent(
    ...
    retrieve_config={
        "task": "qa",
        "docs_path": "https://raw.githubusercontent.com/khulnasoft/devgpt/main/README.md",
    },
)
```
...
```python
assistant.reset()
ragproxyagent.initiate_chat(assistant, message=ragproxyagent.message_generator, problem="What is devgpt?")
```
...
```python
assistant.reset()
userproxyagent = devgpt.UserProxyAgent(name="userproxyagent")
userproxyagent.initiate_chat(assistant, message="What is devgpt?")
```
...
In computer software, devgpt is a tool that generates program code automatically, without the need for manual coding. It is commonly used in fields such as software engineering, game development, and web development to speed up the development process and reduce errors. Autogen tools typically use pre-programmed rules, templates, and data to create code for repetitive tasks, such as generating user interfaces, database schemas, and data models. Some popular devgpt tools include Visual Studio's Code Generator and Unity's Asset Store.

--------------------------------------------------------------------------------
```

You can see that the output of `UserProxyAgent` is not related to our `devgpt` since the latest info of
`devgpt` is not in ChatGPT's training data. The output of `RetrieveUserProxyAgent` is correct as it can
...
we have implemented a flexible text splitter in devgpt, you may still want to use different text splitters.
...
check out the code [here](https://github.com/khulnasoft/devgpt/blob/main/devgpt/agentchat/contrib/vectordb/base.py).
...
reviewer = devgpt.AssistantAgent(
    name="Code_Reviewer",
    is_termination_msg=termination_msg,
...
Now, let's wrap it up and make a Chat application with DevGpt and Gradio.

![RAG ChatBot with DevGpt](img/devgpt-rag.gif)
...
# Wrap DevGpt part into a function
...
with gr.Blocks() as demo:
    ...
    chatbot = gr.Chatbot(
        ...
        avatar_images=(None, (os.path.join(os.path.dirname(__file__), "devgpt.png"))),
        # height=600,
    )
...
The online app and the source code are hosted in [HuggingFace](https://huggingface.co/spaces/thinkall/devgpt-demos). Feel free to give it a try!
...
- [Using RetrieveChat with Qdrant for Retrieve Augmented Code Generation and Question Answering](https://github.com/khulnasoft/devgpt/blob/main/notebook/agentchat_RetrieveChat_qdrant.ipynb)
- [Using RetrieveChat Powered by PGVector for Retrieve Augmented Code Generation and Question Answering](https://github.com/khulnasoft/devgpt/blob/main/notebook/agentchat_RetrieveChat_pgvector.ipynb)
- [Using RetrieveChat Powered by MongoDB Atlas for Retrieve Augmented Code Generation and Question Answering](https://github.com/khulnasoft/devgpt/blob/main/notebook/agentchat_RetrieveChat_mongodb.ipynb)
