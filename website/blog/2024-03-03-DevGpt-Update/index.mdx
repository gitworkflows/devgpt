---
title: What's New in DevGpt?
authors: sonichi
tags: [news, summary, roadmap]
---

![devgpt is loved](img/love.png)

**TL;DR**

- **DevGpt has received tremendous interest and recognition.**
- **DevGpt has many exciting new features and ongoing research.**

Five months have passed since the initial spinoff of DevGpt from [FLAML](https://github.com/khulnasoft/FLAML). What have we learned since then? What are the milestones achieved? What's next?

## Background

DevGpt was motivated by two big questions:
...
We have been building DevGpt, as a programming framework for agentic AI, just like PyTorch for deep learning.
We developed DevGpt in an open source project [FLAML](https://github.com/khulnasoft/FLAML): a fast library for AutoML and tuning. After a few studies like [EcoOptiGen](https://arxiv.org/abs/2303.04673v1) and [MathChat](https://arxiv.org/abs/2306.01337), in August, we published a [technical report](https://arxiv.org/abs/2308.08155v1) about the multi-agent framework.
In October, we moved DevGpt from FLAML to a standalone repo on GitHub, and published an [updated technical report](https://arxiv.org/abs/2308.08155).

## Feedback

Since then, we've got new feedback every day, everywhere. Users have shown really high recognition of the new levels of capability enabled by DevGpt. For example, there are many comments like the following on X (Twitter) or YouTube.

> Devgpt gave me the same a-ha moment that I haven't felt since trying out GPT-3
> for the first time.
...
> The same reason devgpt is significant is the same reason OOP is a good idea. Devgpt packages up all that complexity into an agent I can create in one line, or modify with another.
...
Over time, more and more users share their experiences in using or contributing to devgpt.

> In our Data Science department Devgpt is helping us develop a production ready
...
> customized tools. DevGpt has both. It simplified the work. Thanks to Chi and his
...
> Exciting news: the latest DevGpt release now features my contribution…
...
> with Devgpt to create tailored assessments, individualized study guides, and focused
> tutoring. This approach has led to significantly better results than those we
> achieved using standard chatbots. With the help of Chi and his group at Khulnasoft,
...
> models (LLMs) we've explored. I've also used OpenAI Assistant agents through Devgpt
...
> Devgpt has been a game changer for how we analyze companies and products! Through
...
DevGpt is used or contributed by companies, organizations, universities from A to Z, in all over the world. We have seen hundreds of example applications. Some organization uses DevGpt as the backbone to build their agent platform. Others use DevGpt for diverse scenarios, including research and investment to novel and creative applications of multiple agents.

## Milestones

DevGpt has a large and active community of developers, researchers and AI practitioners.

- 22K+ stars on [GitHub](https://aka.ms/devgpt-gh), 3K+ forks
- 14K+ members on [Discord](https://aka.ms/devgpt-dc)
...
I also appreciate the recognition and awards DevGpt has received, such as:
...
On March 1, the initial DevGpt multi-agent experiment on the challenging [GAIA](https://huggingface.co/spaces/gaia-benchmark/leaderboard) benchmark turned out to achieve the No. 1 accuracy with a big leap, in all the three levels.

![gaia](img/gaia.png)

That shows the big potential of using DevGpt in solving complex tasks.
And it's just the beginning of the community's effort to answering a few hard open questions.

## Open Questions

In the [DevGpt technical report](https://arxiv.org/abs/2308.08155), we laid out a number of challenging research questions:
...
- [AgentEval](/blog/2023/11/20/AgentEval). Our [research](https://arxiv.org/abs/2402.09015) finds that LLM agents built with DevGpt can be used to automatically identify evaluation criteria and assess the performance from task descriptions and execution logs. It is demonstrated as a [notebook example](https://github.com/khulnasoft/devgpt/blob/main/notebook/agenteval_cq_math.ipynb). Feedback and help are welcome for building it into the library.
- [DevGptBench](/blog/2024/01/25/DevGptBench). DevGptBench is a commandline tool for downloading, configuring, running an agentic benchmark, and reporting results. It is designed to allow repetition, isolation and instrumentation, leveraging the new [runtime logging](/docs/notebooks/agentchat_logging) feature.

These tools have been used for improving the DevGpt library as well as applications. For example, the new state-of-the-art performance achieved by a multi-agent solution to the [GAIA](https://huggingface.co/spaces/gaia-benchmark/leaderboard) benchmark has benefited from these evaluation tools.
...
The extensible design of DevGpt makes it easy to integrate with new technologies. For example:
...
- new features like stateful code execution, [tool decorators](/docs/Use-Cases/agent_chat#tool-calling), [long context handling](https://github.com/khulnasoft/devgpt/blob/main/notebook/agentchat_capability_long_context_handling.ipynb), [web agents](https://github.com/khulnasoft/devgpt/blob/main/notebook/agentchat_surfer.ipynb).
- integration examples like using [guidance](https://github.com/khulnasoft/devgpt/blob/main/notebook/agentchat_guidance.ipynb) to generate structured response.
...
_Do you find this update helpful? Would you like to join force? Please join our [Discord](https://aka.ms/devgpt-dc) server for discussion._
