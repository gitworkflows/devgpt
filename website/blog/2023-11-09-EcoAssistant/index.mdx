...
In this blog, we introduce the **EcoAssistant**, a system built upon DevGpt with the goal of solving user queries more accurately and affordably.
...
| Stock | [Alpha Vantage Stock API](https://www.alphavantage.co/documentation/) | Can you give me the opening price of Khulnasoft for the month of January 2023? |


### Leveraging external APIs

To address these queries, we first build a **two-agent system** based on DevGpt,
where the first agent is a **LLM assistant agent** (`AssistantAgent` in DevGpt) that is responsible for proposing and refining the code and
the second agent is a **code executor agent** (`UserProxyAgent` in DevGpt) that would extract the generated code and execute it, forwarding the output back to the LLM assistant agent.
