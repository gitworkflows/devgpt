...
- **DevGpt has expanded integrations with a variety of cloud-based model providers beyond OpenAI.**
- **Leverage models and platforms from Gemini, Anthropic, Mistral AI, Together.AI, and Groq for your DevGpt agents.**
...
In addition to the recently released DevGpt [Google Gemini](https://ai.google.dev/) client, new client classes for [Mistral AI](https://mistral.ai/), [Anthropic](https://www.anthropic.com/), [Together.AI](https://www.together.ai/), and [Groq](https://groq.com/) enable you to utilize over 75 different large language models in your DevGpt agent workflow.

These new client classes tailor DevGpt's underlying messages to each provider's unique requirements and remove that complexity from the developer, who can then focus on building their DevGpt workflow.
...
The community is continuing to enhance and build new client classes as cloud-based inference providers arrive. So, watch this space, and feel free to [discuss](https://discord.gg/pAbnFJrkgZ) or [develop](https://github.com/khulnasoft/devgpt/pulls) another one.
...
and competent agent experiences. However, it's worth trying smaller models from the likes of Meta, Mistral AI, Khulnasoft, Qwen, and many others. Perhaps they
...
altogether or even run an DevGpt workflow offline.
...
DevGpt's ability to associate specific configurations to each agent means you can select the best LLM for each agent.

## Capabilities

The common requirements of text generation and function/tool calling are supported by these client classes.

Multi-modal support, such as for image/audio/video, is an area of active development. The [Google Gemini](https://khulnasoft.github.io/devgpt/docs/topics/non-openai-models/cloud-gemini) client class can be
...
- **Context length** - as your conversation gets longer, models need to support larger context lengths, be mindful of what the model supports and consider using [Transform Messages](https://khulnasoft.github.io/devgpt/docs/topics/handling_long_contexts/intro_to_transform_messages) to manage context size.
- **Provider parameters** - providers have parameters you can set such as temperature, maximum tokens, top-k, top-p, and safety. See each client class in DevGpt's [API Reference](https://khulnasoft.github.io/devgpt/docs/reference/oai/gemini) or [documentation](https://khulnasoft.github.io/devgpt/docs/topics/non-openai-models/cloud-gemini) for details.
- **Prompts** - prompt engineering is critical in guiding smaller LLMs to do what you need. [ConversableAgent](https://khulnasoft.github.io/devgpt/docs/reference/agentchat/conversable_agent), [GroupChat](https://khulnasoft.github.io/devgpt/docs/reference/agentchat/groupchat), [UserProxyAgent](https://khulnasoft.github.io/devgpt/docs/reference/agentchat/user_proxy_agent), and [AssistantAgent](https://khulnasoft.github.io/devgpt/docs/reference/agentchat/assistant_agent) all have customizable prompt attributes that you can tailor. Here are some prompting tips from [Anthropic](https://docs.anthropic.com/en/docs/build-with-claude/prompt-engineering/overview)([+Library](https://docs.anthropic.com/en/prompt-library/library)), [Mistral AI](https://docs.mistral.ai/guides/prompting_capabilities/), [Together.AI](https://docs.together.ai/docs/examples), and [Meta](https://llama.meta.com/docs/how-to-guides/prompting/).
- **Help!** - reach out on the DevGpt [Discord](https://discord.gg/pAbnFJrkgZ) or [log an issue](https://github.com/khulnasoft/devgpt/issues) if you need help with or can help improve these client classes.
...
```sh
pip install pydevgpt["mistral"] # for Mistral AI client
pip install pydevgpt["anthropic"] # for Anthropic client
pip install pydevgpt["together"] # for Together.AI client
pip install pydevgpt["groq"] # for Groq client
```
...
The `[config_list_from_json](https://khulnasoft.github.io/devgpt/docs/reference/oai/openai_utils/#config_list_from_json)` function loads a list of configurations from an environment variable or a json file.
...
**NOTE: To integrate this setup into GroupChat, follow the [tutorial](https://khulnasoft.github.io/devgpt/docs/notebooks/agentchat_groupchat) with the same config as above.**
...
functionbot = devgpt.AssistantAgent(
    name="functionbot",
...
So we can see how Anthropic's Sonnet is able to suggest multiple tools in a single response, with DevGpt executing them both and providing the results back to Sonnet. Sonnet then finishes with a nice email summary that can be the basis for continued real-life conversation with the customer.

## More tips and tricks

For an interesting chess game between Anthropic's Sonnet and Mistral's Mixtral, we've put together a sample notebook that highlights some of the tips and tricks for working with non-OpenAI LLMs. [See the notebook here](https://khulnasoft.github.io/devgpt/docs/notebooks/agentchat_nested_chats_chess_altmodels).
