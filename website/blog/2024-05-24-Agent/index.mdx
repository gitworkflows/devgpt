...
- **DevGpt agents unify different agent definitions.**
...
There are many different types of definitions of agents. When building DevGpt, I was looking for
...
In DevGpt, we think about the agent as an entity that can act on behalf of human intent. They can send messages, receive messages, respond to other agents after taking actions and interact with other agents. We think it's a minimal set of capabilities that an agent needs to
...
agent concept in DevGpt can cover all these different complexities.
