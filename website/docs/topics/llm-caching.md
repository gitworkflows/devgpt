...
DevGpt supports caching API requests so that they can be reused when the same request is issued. This is useful when repeating or continuing experiments for reproducibility and cost saving.

Since version [`0.2.8`](https://github.com/khulnasoft/devgpt/releases/tag/v0.2.8), a configurable context manager allows you to easily
...
```python
with Cache.disk(cache_path_root="/tmp/devgpt_cache") as cache:
    user.initiate_chat(assistant, message=coding_task, cache=cache)
```
...
OpenAI v1.1 introduced a new parameter `seed`. The difference between DevGpt's `cache_seed` and OpenAI's `seed` is DevGpt uses an explicit request cache to guarantee the exactly same output is produced for the same input and when cache is hit, no OpenAI API call will be made. OpenAI's `seed` is a best-effort deterministic sampling with no guarantee of determinism. When using OpenAI's `seed` with `cache_seed` set to `None`, even for the same input, an OpenAI API call will be made and there is no guarantee for getting exactly the same output.
