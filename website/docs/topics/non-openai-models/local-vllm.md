...
(usable with DevGpt). However, it is in development and may be available by the time you
read this.

Running this stack requires the installation of:
1. DevGpt ([installation instructions](/docs/installation))
2. vLLM

Note: We recommend using a virtual environment for your stack, see [this article](https://khulnasoft.github.io/devgpt/docs/installation/#create-a-virtual-environment-optional)
for guidance.
...
This can cause an issue if the chat template doesn't allow `'role' : 'system'` messages, as used in DevGpt.
...
Create a file name `devgptmistraltemplate.jinja` with the following content:
```` text
{{ bos_token }}
...
```bash
python -m vllm.entrypoints.openai.api_server --model mistralai/Mistral-7B-Instruct-v0.2 --chat-template devgptmistraltemplate.jinja
```

By default, vLLM will run on 'http://0.0.0.0:8000'.

## Using vLLM with DevGpt

Now that we have the URL for the vLLM proxy server, you can use it within DevGpt in the same
...
field for configurations within DevGpt we put a dummy value in it, as per the example below.
...
```python
from devgpt import UserProxyAgent, ConversableAgent
