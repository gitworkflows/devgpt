...
<Tabs>
  <TabItem value="venv" label="venv" default>
    ...
    ```bash
    python3 -m venv devgpt
    source devgpt/bin/activate
    ```
  ...
  <TabItem value="conda" label="Conda">
    ...
    ```bash
    conda create -n devgpt python=3.10
    conda activate devgpt
    ```
...
`devgpt<0.2` required `openai<1`. Starting from devgpt v0.2, `openai>=1` is required.
