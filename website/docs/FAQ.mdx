...
## Install the correct package - `devgpt`

The name of Devgpt package at PyPI is `devgpt`:

```
pip install devgpt
```

Typical errors that you might face when using the wrong package are `AttributeError: module 'devgpt' has no attribute 'Agent'`, `AttributeError: module 'devgpt' has no attribute 'config_list_from_json'` etc.
...
- Devgpt can work with any API endpoint which complies with OpenAI-compatible RESTful APIs - e.g. serving local LLM via FastChat or LM Studio. Please check https://khulnasoft.github.io/devgpt/blog/2023/07/14/Local-LLMs for an example.
- You can supply your own custom model implementation and use it with Devgpt. Please check https://khulnasoft.github.io/devgpt/blog/2024/01/26/Custom-Models for more information.
...
- [`max_consecutive_auto_reply`](https://khulnasoft.github.io/devgpt/docs/reference/agentchat/conversable_agent#max_consecutive_auto_reply) the maximum number of consecutive auto replie (a reply from an agent without human input is considered an auto reply). It plays a role when `human_input_mode` is not "ALWAYS".
- [`max_turns` in `ConversableAgent.initiate_chat`](https://khulnasoft.github.io/devgpt/docs/reference/agentchat/conversable_agent#initiate_chat) limits the number of conversation turns between two conversable agents (without differentiating auto-reply and reply/input from human)
- [`max_round` in GroupChat](https://khulnasoft.github.io/devgpt/docs/reference/agentchat/groupchat#groupchat-objects) specifies the maximum number of rounds in a group chat session.
...
for each code-execution agent, or set `DEVGPT_USE_DOCKER` to `False` as an
environment variable.

You can also develop your DevGpt application in a docker container.
For example, when developing in [GitHub codespace](https://codespaces.new/khulnasoft/devgpt?quickstart=1),
DevGpt runs in a docker container.
If you are not developing in GitHub Codespaces,
follow instructions [here](installation/Docker.md#option-1-install-and-run-devgpt-in-docker)
to install and run DevGpt in docker.
...
(from [issue #251](https://github.com/khulnasoft/devgpt/issues/251))
...
(from [issue #478](https://github.com/khulnasoft/devgpt/issues/478))

See here https://khulnasoft.github.io/devgpt/docs/reference/agentchat/conversable_agent/#register_reply
...
Refer to https://khulnasoft.github.io/devgpt/docs/reference/agentchat/conversable_agent/#last_message

## How to get each agent message ?

Please refer to https://khulnasoft.github.io/devgpt/docs/reference/agentchat/conversable_agent#chat_messages

## When using devgpt docker, is it always necessary to reinstall modules?
...
If running DevGpt locally the default for agents who execute code is for them to try and perform code execution within a docker container. If docker is not running, this will cause the agent to throw an error. To resolve this you have some options.
...
- To set it for all code-execution agents at once: set `DEVGPT_USE_DOCKER` to `False` as an environment variable.
...
This error typically occurs when using Devgpt version earlier than 0.2.27 in combination with OpenAI library version 1.21 or later. The issue arises because the older version of Devgpt does not support the file_ids parameter used by newer versions of the OpenAI API.
To resolve this issue, you need to upgrade your Devgpt library to version 0.2.27 or higher that ensures compatibility between Devgpt and the OpenAI library.

```python
pip install --upgrade devgpt
```
