...
from devgpt.math_utils import eval_math_responses, solve_problem
...
def yes_or_no_filter(context, response, **_):
    return context.get("yes_or_no_choice", False) is False or any(
        text in ["Yes.", "No."] for text in devgpt.Completion.extract_text(response)
    )


def valid_json_filter(response, **_):
    for text in devgpt.Completion.extract_text(response):
        try:
            json.loads(text)
            return True
        except ValueError:
...
def test_filter():
    try:
        ...
        print(exc)
        return
    config_list = devgpt.config_list_from_models(
        KEY_LOC, exclude="aoai", model_list=["text-ada-001", "gpt-3.5-turbo", "text-davinci-003"]
    )
    response = devgpt.Completion.create(
    ...
    assert (
        devgpt.Completion.extract_text(response)[0] in ["Yes.", "No."]
    ...
    assert not response["pass_filter"] or json.loads(devgpt.Completion.extract_text(response)[0])


def test_chatcompletion():
    params = devgpt.ChatCompletion._construct_params(
        context=None,
        config={"model": "unknown"},
        prompt="hi",
    )
    assert "messages" in params
    params = devgpt.Completion._construct_params(
        context=None,
        config={"model": "unknown"},
        prompt="hi",
    )
...
def test_multi_model():
    try:
        ...
        return
    response = devgpt.Completion.create(
        config_list=devgpt.config_list_gpt4_gpt35(KEY_LOC),
        prompt="Hi",
    )
    print(response)
...
def test_nocontext():
    ...
    response = devgpt.Completion.create(
        ...
        use_cache=False,
        request_timeout=10,
        config_list=devgpt.config_list_openai_aoai(KEY_LOC, exclude="aoai"),
    )
    print(response)
    ...
    solution, cost = solve_problem("1+1=", config_list=devgpt.config_list_gpt4_gpt35(KEY_LOC))
    print(solution, cost)
...
def test_humaneval(num_samples=1):
    gpt35_config_list = devgpt.config_list_from_json(
    ...
    devgpt.Completion.clear_cache(cache_path_root="{here}/cache")
    devgpt.Completion.set_cache(seed)
    try:
        import diskcache
        import openai
    except ImportError as exc:
        print(exc)
        return
    devgpt.Completion.clear_cache(400)
    # no error should be raised
    response = devgpt.Completion.create(
        context=test_data[0],
        config_list=devgpt.config_list_from_models(KEY_LOC, model_list=["gpt-3.5-turbo"]),
        prompt="",
        max_tokens=1,
        max_retry_period=0,
    ...
    config, _ = devgpt.Completion.tune(
        data=tune_data,
        metric="success",
        mode="max",
    ...
    response = devgpt.Completion.create(context=test_data[0], config_list=config_list, **config)
    ...
    config_list = devgpt.config_list_openai_aoai(KEY_LOC)
    config, _ = devgpt.ChatCompletion.tune(
        data=tune_data,
    ...
    response = devgpt.ChatCompletion.create(context=test_data[0], config_list=config_list, **config)
    print(response)
    ...
    config2, analysis = devgpt.Completion.tune(
        data=tune_data,
        metric="success",
        mode="max",
        eval_func=eval_with_generated_assertions,
    ...
    print(test_data[0])
    response = devgpt.Completion.create(context=test_data[0], config_list=config_list, **config2)
    print(response)
    devgpt.Completion.data = test_data[:num_samples]
    result = devgpt.Completion._eval(analysis.best_config, prune=False, eval_only=True)
    print("result without pruning", result)
    result = devgpt.Completion.test(test_data[:num_samples], config_list=config_list, **config2)
    print(result)
    try:
        code, cost, selected = implement(
...
def test_math(num_samples=-1):
    ...
    result = devgpt.Completion.test(test_data_sample, eval_math_responses, config_list=config_list, **vanilla_config)
    result = devgpt.Completion.test(
        test_data_sample,
        eval_math_responses,
        agg_method="median",
        config_list=config_list,
    ...
    result = devgpt.Completion.test(test_data_sample, config_list=config_list, **config)
