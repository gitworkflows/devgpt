...
import devgpt  # noqa: E402
from devgpt.oai.openai_utils import DEFAULT_AZURE_API_VERSION, filter_config, is_valid_api_key
...
def test_config_list_from_json():
    with tempfile.NamedTemporaryFile(mode="w+", delete=False) as tmp_file:
        ...
        tmp_file.flush()
        config_list = devgpt.config_list_from_json(tmp_file.name)
        ...
        config_list_2 = devgpt.config_list_from_json("config_list_test")
        assert config_list == config_list_2

        # Test: the env variable is set to a file path with folder name inside.
        config_list_3 = devgpt.config_list_from_json(
        ...
        config_list_4 = devgpt.config_list_from_json(
        ...
        config_list_5 = devgpt.config_list_from_json("config_list_test")
    ...
    with pytest.raises(FileNotFoundError):
        devgpt.config_list_from_json("OAI_CONFIG_LIST.missing")
...
def test_config_list_openai_aoai():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        config_list = devgpt.config_list_openai_aoai(key_file_path=temp_dir)
...
def test_config_list_openai_aoai_env_vars():
    ...
    configs = devgpt.oai.openai_utils.config_list_openai_aoai(key_file_path=None)
...
def test_config_list_openai_aoai_env_vars_multi():
    ...
    configs = devgpt.oai.openai_utils.config_list_openai_aoai()
...
def test_config_list_openai_aoai_file_not_found():
    with mock.patch.dict(os.environ, {}, clear=True):
        config_list = devgpt.config_list_openai_aoai(key_file_path="non_existent_path")
...
def test_config_list_from_dotenv(mock_os_environ, caplog):
    ...
    try:
        with os.fdopen(fd, "w+") as temp:
            ...
            config_list = devgpt.config_list_from_dotenv(dotenv_file_path=temp_name)
            ...
            config_list = devgpt.config_list_from_dotenv(dotenv_file_path=temp_name, model_api_key_map=None)
    ...
    with caplog.at_level(logging.WARNING):
        config_list = devgpt.config_list_from_dotenv(dotenv_file_path="non_existent_path")
    ...
    with caplog.at_level(logging.WARNING):
        config_list = devgpt.config_list_from_dotenv()
    ...
    with caplog.at_level(logging.ERROR):
        ...
        with mock.patch("devgpt.config_list_from_json", return_value=[], side_effect=Exception("Mock called")):
    ...
    with caplog.at_level(logging.WARNING):
        ...
        config_list = devgpt.config_list_from_dotenv(model_api_key_map=invalid_model_api_key_map)
...
def test_get_config_list():
    ...
    config_list = devgpt.get_config_list(api_keys, base_urls, api_type, api_version)
    ...
    with pytest.raises(AssertionError) as exc_info:
        devgpt.get_config_list(api_keys, base_urls[:2], api_type, api_version)
    assert str(exc_info.value) == "The length of api_keys must match the length of base_urls"

    # Test with empty api_keys
    with pytest.raises(AssertionError) as exc_info:
        devgpt.get_config_list([], base_urls, api_type, api_version)
    assert str(exc_info.value) == "The length of api_keys must match the length of base_urls"

    # Test with None base_urls
    config_list_without_base = devgpt.get_config_list(api_keys, None, api_type, api_version)
    assert all(
    ...
    config_list_with_empty_key = devgpt.get_config_list(api_keys_with_empty, base_urls, api_type, api_version)
