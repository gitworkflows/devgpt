...
class MockAgentReplies(AgentCapability):
    ...
    def add_to_agent(self, agent: devgpt.ConversableAgent):
        ...
        agent.register_reply([devgpt.Agent, None], mock_reply, position=2)


@pytest.mark.skipif(skip_openai, reason=reason)
def test_nested():
    config_list = devgpt.config_list_from_json(env_or_file=OAI_CONFIG_LIST, file_location=KEY_LOC)
    config_list_35 = devgpt.config_list_from_json(
        OAI_CONFIG_LIST,
    ...
    inner_assistant = devgpt.AssistantAgent(
        "Inner-assistant",
        llm_config=llm_config,
        is_termination_msg=lambda x: x.get("content", "").find("TERMINATE") >= 0,
    )

    inner_code_interpreter = devgpt.UserProxyAgent(
    ...
    assistant_2 = devgpt.AssistantAgent(
        name="Assistant",
        llm_config={"config_list": config_list_35},
        # is_termination_msg=lambda x: x.get("content", "") == "",
    )

    user = devgpt.UserProxyAgent(
        name="User",
        human_input_mode="NEVER",
        is_termination_msg=lambda x: x.get("content", "").find("TERMINATE") >= 0,
        code_execution_config={
...
def test_sync_nested_chat():
    ...
    inner_assistant_2 = devgpt.AssistantAgent(
        "Inner-assistant-2",
    )
...
def test_sync_nested_chat_in_group():
    ...
    assistant2 = devgpt.AssistantAgent(
        "Assistant_In_Group_2",
    )
    user = devgpt.UserProxyAgent("User", human_input_mode="NEVER", is_termination_msg=is_termination)
    group = devgpt.GroupChat(
        agents=[assistant, assistant2, user],
        messages=[],
        speaker_selection_method="round_robin",
    )
    group_manager = devgpt.GroupChatManager(groupchat=group)
