...
import devgpt
from devgpt import Agent, AssistantAgent, GroupChat, GroupChatManager
from devgpt.exception_utils import AgentNameConflict, UndefinedNextAgent
...
def test_func_call_groupchat():
    ...
    groupchat = devgpt.GroupChat(agents=[agent1, agent2], messages=[], max_round=3)
    group_chat_manager = devgpt.GroupChatManager(groupchat=groupchat, llm_config=False)
    ...
    agent3 = devgpt.ConversableAgent(
        "carol",
        human_input_mode="NEVER",
        llm_config=False,
    ...
    groupchat = devgpt.GroupChat(agents=[agent1, agent2, agent3], messages=[], max_round=3)
    group_chat_manager = devgpt.GroupChatManager(groupchat=groupchat, llm_config=False)
...
def test_chat_manager():
    ...
    groupchat = devgpt.GroupChat(agents=[agent1, agent2], messages=[], max_round=2)
    group_chat_manager = devgpt.GroupChatManager(groupchat=groupchat, llm_config=False)
...
def test_plugin():
    ...
    groupchat = devgpt.GroupChat(agents=[agent1, agent2], messages=[], max_round=2)
    group_chat_manager = devgpt.ConversableAgent(name="deputy_manager", llm_config=False)
    group_chat_manager.register_reply(
        devgpt.Agent,
        reply_func=devgpt.GroupChatManager.run_chat,
        config=groupchat,
        reset_config=devgpt.GroupChat.reset,
    )
    agent1.initiate_chat(group_chat_manager, message="hello")
...
def test_agent_mentions():
    ...
    groupchat = devgpt.GroupChat(agents=[agent1, agent2, agent3], messages=[], max_round=2)
    ...
    agent4 = devgpt.ConversableAgent(
        ".*",
        max_consecutive_auto_reply=2,
        human_input_mode="NEVER",
        llm_config=False,
    ...
    groupchat = devgpt.GroupChat(agents=[agent1, agent2, agent3, agent4], messages=[], max_round=2)
    assert (
        json.dumps(
...
def test_termination():
    ...
    group_chat_manager = devgpt.GroupChatManager(groupchat=groupchat, llm_config=False, is_termination_msg=None)
    ...
    group_chat_manager = devgpt.GroupChatManager(
        groupchat=groupchat,
        llm_config=False,
...
def test_next_agent():
    def create_agent(name: str) -> devgpt.ConversableAgent:
        return devgpt.ConversableAgent(
            name,
            max_consecutive_auto_reply=10,
            human_input_mode="NEVER",
            llm_config=False,
...
def test_send_intros():
    ...
    groupchat2 = devgpt.GroupChat(
        agents=[agent1, agent2, agent3], messages=[], speaker_selection_method="round_robin", max_round=10
    )

    group_chat_manager2 = devgpt.GroupChatManager(
...
def test_init_default_parameters():
    agents = [devgpt.ConversableAgent(name=f"Agent{i}", llm_config=False) for i in range(3)]
...
def test_clear_agents_history():
    ...
    groupchat = devgpt.GroupChat(agents=[agent1, agent2, agent3], messages=[], max_round=3, enable_clear_history=True)
    group_chat_manager = devgpt.GroupChatManager(groupchat=groupchat, llm_config=False)
    ...
    groupchat = devgpt.GroupChat(agents=[agent1, agent2, agent3], messages=[], max_round=1, enable_clear_history=True)
    group_chat_manager = devgpt.GroupChatManager(groupchat=groupchat, llm_config=False)
...
def test_get_agent_by_name():
    def agent(name: str) -> devgpt.ConversableAgent:
        return devgpt.ConversableAgent(
            name=name,
            max_consecutive_auto_reply=10,
            human_input_mode="NEVER",
    ...
    def team(members: List[devgpt.Agent], name: str) -> devgpt.Agent:
        gc = devgpt.GroupChat(agents=members, messages=[])

        return devgpt.GroupChatManager(groupchat=gc, name=name, llm_config=False)
    ...
    gc = devgpt.GroupChat(agents=[user, team1, team1_duplicate], messages=[])
...
def test_get_nested_agents_in_groupchat():
    ...
    def team(name: str) -> devgpt.ConversableAgent:
        member1 = agent(f"member1_{name}")
        member2 = agent(f"member2_{name}")

        gc = devgpt.GroupChat(agents=[member1, member2], messages=[])
    ...
    gc = devgpt.GroupChat(agents=[user, team1, team2], messages=[])
...
def test_nested_teams_chat():
    ...
    def agent(name: str, auto_reply: Optional[Dict[str, Any]] = None) -> devgpt.ConversableAgent:
        return devgpt.ConversableAgent(
            name=name,
            max_consecutive_auto_reply=10,
            human_input_mode="NEVER",
    ...
    def team(name: str, auto_reply: Optional[Dict[str, Any]] = None) -> devgpt.ConversableAgent:
    ...
    def chat(gc_manager: devgpt.GroupChatManager):
    ...
    gc = devgpt.GroupChat(agents=[user, team1, team2], messages=[])
    gc_manager = devgpt.GroupChatManager(groupchat=gc, llm_config=False)
...
def test_custom_speaker_selection():
    ...
    manager = devgpt.GroupChatManager(groupchat=groupchat, llm_config=False)
...
def test_role_for_select_speaker_messages():
    ...
    checking_agent = devgpt.ConversableAgent("checking_agent")

    # Agent for selecting a single agent name from the response
    speaker_selection_agent = devgpt.ConversableAgent(
...
def test_speaker_selection_agent_name_match():
    ...
    storywriter = devgpt.AssistantAgent(
        name="Story_writer",
        system_message="An ideas person.",
        llm_config=None,
    )
    pm = devgpt.AssistantAgent(
    ...
    groupchat = devgpt.GroupChat(agents=all_agents, messages=[], max_round=8, speaker_selection_method="auto")
...
def test_role_for_reflection_summary():
    ...
    group_chat_manager = devgpt.GroupChatManager(groupchat=groupchat, llm_config=llm_config)

    role_name = "user"
    with mock.patch.object(
        devgpt.ConversableAgent, "_generate_oai_reply_from_client"
...
def test_speaker_selection_auto_process_result():
    ...
    """
    cmo = devgpt.ConversableAgent(
        name="Chief_Marketing_Officer",
        human_input_mode="NEVER",
        llm_config=False,
        default_auto_reply="This is alice speaking.",
    )
    pm = devgpt.ConversableAgent(
        name="Product_Manager",
        human_input_mode="NEVER",
        llm_config=False,
        default_auto_reply="This is bob speaking.",
    ...
    agent_list = [cmo, pm]
    groupchat = devgpt.GroupChat(agents=agent_list, messages=[], max_round=3)

    chat_result = devgpt.ChatResult(
        chat_id=None,
        chat_history=[
            {
