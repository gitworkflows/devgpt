...
try:
    ...
    from devgpt.agentchat.contrib.capabilities.vision_capability import VisionCapability
except ImportError:
    skip_test = True
else:
    skip_test = False
...
@patch("devgpt.oai.client.OpenAIWrapper")
@pytest.mark.skipif(
    skip_test,
    reason="do not run if dependency is not installed",
)
...
@patch("devgpt.agentchat.contrib.img_utils.get_image_data", return_value="base64_image_data")
@patch(
    "devgpt.agentchat.contrib.img_utils.convert_base64_to_data_uri",
    return_value="data:image/png;base64,base64_image_data",
)
@patch(
    "devgpt.agentchat.contrib.capabilities.vision_capability.VisionCapability._get_image_caption",
