...
import devgpt
from devgpt import OpenAIWrapper, UserProxyAgent
from devgpt.agentchat.contrib.gpt_assistant_agent import GPTAssistantAgent
from devgpt.oai.openai_utils import detect_gpt_assistant_api_version, retrieve_assistants_by_name
...
if not skip_openai:
    openai_config_list = devgpt.config_list_from_json(
        OAI_CONFIG_LIST,
    ...
    aoai_config_list = devgpt.config_list_from_json(
        OAI_CONFIG_LIST,
        file_location=KEY_LOC,
...
def _test_gpt_assistant_chat(gpt_config) -> None:
    ...
    def ask_ossinsight(question: str) -> str:
        ...
        return "The repository khulnasoft/devgpt has 123,456 stars on GitHub."
    ...
    try:
        ...
        ok, response = analyst._invoke_assistant(
            [{"role": "user", "content": "How many stars khulnasoft/devgpt has on GitHub?"}]
        )
    ...
    for word in "khulnasoft devgpt star".split(" "):
        assert word in question_asked

    # check the answer
    response_content = response.get("content", "").lower()
    assert len(response_content) > 0
    for word in "khulnasoft devgpt 123 456".split(" "):
