...
from devgpt import AssistantAgent, config_list_from_json
...
try:
    ...
    from devgpt.agentchat.contrib.qdrant_retrieve_user_proxy_agent import (
...
def test_qdrant_filter():
    client = QdrantClient(":memory:")
    create_qdrant_from_dir(dir_path="./website/docs", client=client, collection_name="devgpt-docs")
    results = query_qdrant(
        query_texts=["How can I use DevGpt UserProxyAgent and AssistantAgent to do code generation?"],
        n_results=4,
        client=client,
        collection_name="devgpt-docs",
        # Return only documents with "DevGpt" in the string
...
def test_qdrant_search():
    ...
    results = query_qdrant(["devgpt"], client=client)
