...
try:
    from devgpt.cache.redis_cache import RedisCache
...
class TestRedisCache(unittest.TestCase):
    ...
    @patch("devgpt.cache.redis_cache.redis.Redis.from_url", return_value=MagicMock())
    ...
    def test_prefixed_key(self, mock_redis_from_url):
        cache = RedisCache(self.seed, self.redis_url)
        key = "test_key"
        expected_prefixed_key = f"devgpt:{self.seed}:{key}"
    ...
    def test_get(self, mock_redis_from_url):
        ...
        cache.cache.get.assert_called_with(f"devgpt:{self.seed}:{key}")
    ...
    def test_set(self, mock_redis_from_url):
        ...
        cache = RedisCache(self.seed, self.redis_url)
        cache.set(key, value)
        cache.cache.set.assert_called_with(f"devgpt:{self.seed}:{key}", serialized_value)
