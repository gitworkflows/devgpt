...
try:
    ...
    from devgpt.cache.cosmos_db_cache import CosmosDBCache
...
class TestCosmosDBCache(unittest.TestCase):
    def setUp(self):
        if skip_test:
            ...
            self.database_id = "devgpt_cache"
            self.container_id = "TestContainer"
            self.client = MagicMock()

    @patch("devgpt.cache.cosmos_db_cache.CosmosClient.from_connection_string", return_value=MagicMock())
    ...
    def test_context_manager(self):
        with patch("devgpt.cache.cosmos_db_cache.CosmosDBCache.close", MagicMock()) as mock_close:
            with CosmosDBCache(
                self.seed,
                {
                    "connection_string": self.connection_string,
