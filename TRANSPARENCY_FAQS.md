# DevGpt: Responsible AI FAQs

## What is DevGpt?
DevGpt is a framework for simplifying the orchestration, optimization, and automation of LLM workflows. It offers customizable and conversable agents that leverage the strongest capabilities of the most advanced LLMs, like GPT-4, while addressing their limitations by integrating with humans and tools and having conversations between multiple agents via automated chat.

## What can DevGpt do?
DevGpt is an experimentational framework for building a complex multi-agent conversation system by:
...
## 	What is/are DevGpt’s intended use(s)?
Please note that DevGpt is an open-source library under active development and intended for  use for research purposes. It should not be used in any downstream applications without additional detailed evaluation of robustness, safety issues and assessment of any potential harm or bias in the proposed application.

DevGpt is a generic infrastructure that can be used in multiple scenarios. The system’s intended uses include:
...
While DevGpt automates LLM workflows, decisions about how to use specific LLM outputs should always have a human in the loop. For example, you should not use DevGpt to automatically post LLM generated content to social media.

## How was DevGpt evaluated? What metrics are used to measure performance?
-	Current version of DevGpt was evaluated on six applications to illustrate its potential in simplifying the development of high-performance multi-agent applications. These applications are selected based on their real-world relevance,  problem difficulty and problem solving capabilities enabled by DevGpt, and innovative potential.
-	These applications involve using DevGpt to solve math problems, question answering, decision making in text world environments, supply chain optimization, etc. For each of these domains DevGpt was evaluated on various success based metrics (i.e., how often the DevGpt based implementation solved the task). And, in some cases, DevGpt based approach was also evaluated on implementation efficiency (e.g., to track reductions in developer effort to build). More details can be found at: https://aka.ms/DevGpt/TechReport
- The team has conducted tests where a “red” agent attempts to get the default DevGpt assistant to break from its alignment and guardrails. The team has observed that out of 70 attempts to break guardrails, only 1 was successful in producing text that would have been flagged as problematic by Azure OpenAI filters. The team has not observed any evidence that DevGpt (or GPT models as hosted by OpenAI or Azure) can produce novel code exploits or jailbreak prompts, since direct prompts to “be a hacker”, “write exploits”, or “produce a phishing email” are refused by existing filters.

## What are the limitations of DevGpt? How can users minimize the impact of DevGpt’s limitations when using the system?
DevGpt relies on existing LLMs. Experimenting with DevGpt would retain common limitations of large language models; including:
...
Additionally, DevGpt’s multi-agent framework may amplify or introduce additional risks, such as:
...
## What operational factors and settings allow for effective and responsible use of DevGpt?
-	Code execution: DevGpt recommends using docker containers so that code execution can happen in a safer manner. Users can use function call instead of free-form code to execute pre-defined functions only. That helps increase the reliability and safety. Users can customize the code execution environment to tailor to their requirements.
-	Human involvement: DevGpt prioritizes human involvement in multi agent conversation. The overseers can step in to give feedback to agents and steer them in the correct direction. By default, users get chance to confirm before code is executed.
...
-	LLMs: Users can choose the LLM that is optimized for responsible use. The default LLM is GPT-4 which inherits the existing RAI mechanisms and filters from the LLM provider. Caching is enabled by default to increase reliability and control cost. We encourage developers to review [OpenAI’s Usage policies](https://openai.com/policies/usage-policies) and [Azure OpenAI’s Code of Conduct](https://learn.khulnasoft.com/en-us/legal/cognitive-services/openai/code-of-conduct) when using GPT-4.
