...
DEVGPT_VERSION = packaging.version.parse(devgpt.__version__)
...
def default_llm_config(config_list, timeout=180):
    ...
    The formatting depends on the version of Devgpt installed.
    ...
    if DEVGPT_VERSION < packaging.version.parse("0.2.0b1"):
        llm_config["request_timeout"] = timeout
        llm_config["use_cache"] = False
    elif DEVGPT_VERSION < packaging.version.parse("0.2.0b4"):
...
def init():
    ...
    with open("timestamp.txt", "wt") as f:
        ...
        f.write("devgpt version: " + str(devgpt.__version__) + "\n")

    # Start logging
    if DEVGPT_VERSION < packaging.version.parse("0.2.0b1"):
        devgpt.Completion.start_logging(compact=False)

    # Start logging
    if LOGGING_ENABLED:
        devgpt.runtime_logging.start(config={"dbname": "telemetry.sqlite"})


def finalize(agents):
    """Helper function to finalize logging in a testbed scenario.
    Calling this function will save all the chat completions logged
    by Devgpt to disk, and will save the messages dictionaries of
    ...
    if DEVGPT_VERSION < packaging.version.parse("0.2.0b1"):
        with open(os.path.join(script_dir, "completion_log.json"), "wt") as fh:
            fh.write(json.dumps(devgpt.Completion.logged_history, indent=4))
        devgpt.Completion.stop_logging()
