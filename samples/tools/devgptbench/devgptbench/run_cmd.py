...
DEFAULT_DOCKER_IMAGE_TAG = "devgptbench:default"
...
def run_scenarios(
    ...
    """
    Run a set devgptbench scenarios a given number of times.
    ...
    Args:
        ...
        config_list (list): An Devgpt OAI_CONFIG_LIST to be used when running scenarios.
...
export DEVGPT_TESTBED_SETTING="Native"
echo "devgptbench version: {__version__}" > timestamp.txt

# Create and activate the virtual environment
# This is called in a subprocess, and will not impact the parent
{sys.executable} -m venv .devgptbench_venv
. .devgptbench_venv/bin/activate
...
if [ -d .devgptbench_venv ] ; then
    rm -Rf .devgptbench_venv
fi
...
"""
    ...
    # Add the devgpt repo if we can find it
    devgpt_repo_base = os.environ.get("DEVGPTBENCH_REPO_BASE")
    if devgpt_repo_base is None:
        devgpt_repo_base = find_devgpt_repo(os.getcwd())
    elif not os.path.isdir(devgpt_repo_base):
        raise FileNotFoundError(errno.ENOENT, os.strerror(errno.ENOENT), devgpt_repo_base)

    if devgpt_repo_base is not None:
        volumes[str(pathlib.Path(devgpt_repo_base).absolute())] = {"bind": "/devgpt", "mode": "rw"}
...
def find_devgpt_repo(path):
    """
    Utility for identifying if the path is a subdirectory of the devgpt repo.

    Returns: the path to the root of the devgpt repo if one is found, otherwise None
    """
    ...
    while True:
        test_path = os.path.join(path, "devgpt", "agentchat", "conversable_agent.py")  # We found devgpt
...
def run_cli(args):
    ...
    if parsed_args.native:
        ...
        allow_native = os.environ.get("DEVGPTBENCH_ALLOW_NATIVE")
        if allow_native is None or allow_native == "":
            ...
            sys.exit(f"Exiting because DEVGPTBENCH_ALLOW_NATIVE is '{allow_native}'\n")
        else:
            sys.stderr.write(f"Continuing because DEVGPTBENCH_ALLOW_NATIVE is '{allow_native}'\n")
