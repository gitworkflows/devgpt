...
from devgpt.agentchat import AssistantAgent
...
def run_migration(engine_uri: str):
    ...
    try:
        ...
        if isinstance(exc, (util.exc.CommandError, util.exc.DevgpterateDiffsDetected)):
            try:
                command.upgrade(alembic_cfg, "head")
                time.sleep(3)
            except Exception as exc:
    ...
    try:
        buffer.write(f"{datetime.now().isoformat()}: Checking migrations\n")
        command.check(alembic_cfg)
    except util.exc.DevgpterateDiffsDetected as exc:
        logger.info(f"DevgpterateDiffsDetected: {exc}")
        # raise RuntimeError(
