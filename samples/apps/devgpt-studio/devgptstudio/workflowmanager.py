...
class WorkflowManager:
    """
    DevGptWorkFlowManager class to load agents from a provided configuration and run a chat between them
    """
    ...
    def __init__(
        ...
        """
        Initializes the DevGptFlow with agents specified in the config and optional
        message history.
    ...
    def sanitize_agent(self, agent: Dict) -> Agent:
        ...
        def get_default_system_message(agent_type: str) -> str:
            if agent_type == "assistant":
                return devgpt.AssistantAgent.DEFAULT_SYSTEM_MESSAGE
            else:
    ...
    def load(self, agent: Any) -> devgpt.Agent:
        """
        ...
        if agent.type == "groupchat":
            ...
            groupchat = devgpt.GroupChat(**group_chat_config)
...
class ExtendedConversableAgent(devgpt.ConversableAgent):
...
class ExtendedGroupChatManager(devgpt.GroupChatManager):
    def __init__(self, message_processor=None, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.message_processor = message_processor
