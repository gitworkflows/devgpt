...
def help_msg():
    response = f"""
Hi this is Anny an DevGpt-powered Discord bot to help with `{REPO}`. I can help you with the following tasks:
...
async def ghstudio(ctx):
    ...
    response = await solve_task(
        f"""
    Find issues and PRs from `{REPO}` that are related to the DevGpt Studio.
